 ...
 */
public class GetSkillsTask extends AbstractTask<String,Void,ProfileSkillList,ProfileActivity> {

    private final MyPreferenceHelper helper;
    private final MyPreferences pref;

    public GetSkillsTask(ProfileActivity activity) {
        super(activity);
    ...
    protected void onPostExecute(ProfileSkillList skillList) {
        ...
        if(skillList != null){
            ((PerfilRead) weakReference.get()).onProfileSkillSuccess(skillList);
        } else{
            weakReference.get().showSnackbarSimpleMessage("No se puede obtener perfil skills");
        }
    }
