 ...
 */
public class PostSummaryTask extends AbstractTask<String,Void,String,ProfileActivity> {

    private final MyPreferenceHelper helper;
    private final MyPreferences pref;

    public PostSummaryTask(ProfileActivity activity) {
        super(activity);
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            restClient.post("/users/"+helper.getProfessional().getEmail()+"/profile/summary?token="+token, body, withAuth(ctx));
    ...
    protected void onPostExecute(String summary) {
        ...
        if(summary != null){
            ((ProfileCreate) weakReference.get()).onProfileSummaryCreationSuccess();
        } else{
