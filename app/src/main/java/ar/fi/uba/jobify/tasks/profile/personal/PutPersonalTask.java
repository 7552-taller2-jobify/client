 ...
 */
public class PutPersonalTask extends AbstractTask<String,Void,String,ProfileActivity> {

    private final MyPreferenceHelper helper;
    private final MyPreferences pref;

    public PutPersonalTask(ProfileActivity activity) {
        super(activity);
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            restClient.put("/users/"+helper.getProfessional().getEmail()+"/profile/personal?token="+token, body, withAuth(ctx));
    ...
    protected void onPostExecute(String personal) {
        ...
        if(personal != null){
            ((ProfileEdit) weakReference.get()).onProfilePersonalModificationSuccess();
        } else{
